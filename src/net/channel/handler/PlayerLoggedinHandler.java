...
public class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (player.getMapId() == 0 && player.getLevel() < 2) {
            for (ChannelServer cs : ChannelServer.getAllInstances()) {
                cs.broadcastPacket(MaplePacketCreator.serverNotice(6, "Let's welcome " + player.getName() + " to NostalgicMS!"));
            }
        }
        //									THIS COMMAND DO DISPLAY GAME MASTERS.
        //									THIS COMMAND WILL DISPLAY WHEN A CHARACTER LEAVES CS/MTS.
        if (player.getMapId() > 2 && player.getLevel() > 1) {
            if (!player.isGM()) {
                for (ChannelServer cs : ChannelServer.getAllInstances()) {
                    cs.broadcastPacket(MaplePacketCreator.serverNotice(6, player.getName() + " has joined the game."));
                }
            }
        }
        if (!player.rules) {
            NPCScriptManager.getInstance().startRules(c);
        }
    }
}