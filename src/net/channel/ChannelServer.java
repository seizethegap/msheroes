...
public class ChannelServer implements Runnable, ChannelServerMBean {
            ...
            synchronized (cwi) {
                ...
                synchronized (wci) {
                    try {
                        ...
                        shopMesoRate = Integer.parseInt(props.getProperty("world.shopMesoRate"));
                        ...
                        orbgain = Boolean.parseBoolean(props.getProperty("world.orbgain", "false"));
    ...
    public void run() {
        ...
        port = Integer.parseInt(props.getProperty("channel.net.port")) + getChannel();
    ...
    public static Collection<MapleCharacter> getAllPlayers() {
        List<MapleCharacter> ret = new LinkedList<>();
        for (ChannelServer cs : ChannelServer.getAllInstances()) {
            ret.addAll(cs.getPlayerStorage().getAllCharacters());
        }
        return ret;
    }
    
    public static MapleCharacter getPlayerById(int id) {
        Collection<MapleCharacter> ret = getAllPlayers();
        for (MapleCharacter mc : ret) {
            if (mc.getId() == id) {
                return mc;
            }
        }
