...
public class GMCommand {
    ...
    public static boolean executeGMCommand(MapleClient c, MessageCallback mc, String line) {
        ...
        if (splitted[0].equals("!ban")) {
            try {
                ...
                if (target != null) {
                    if (target.gmLevel() < 3 || player.gmLevel() > 4) {
                        ...
                        mc.dropMessage("You may not ban Game Masters.");
                    }
                } else {
            ...
            mc.dropMessage("Done.");
        } else if (splitted[0].equals("!savegame") || splitted[0].equals("!saveall")) {
            ...
            player.getMap().warpMap(Integer.parseInt(splitted[1]));
        } else if (splitted[0].equals("!reloadmap")) {
            int map = Integer.parseInt(splitted[1]);
            Collection<MapleCharacter> lolz = player.getMap().getCharacters();
            c.getChannelServer().getMapFactory().disposeMap(map);
            MapleMap newmap = c.getChannelServer().getMapFactory().getMap(map);
            for (MapleCharacter ch : lolz) {
                ch.changeMap(newmap, newmap.getPortal(0));
            }
        ...
        } else if (splitted[0].equals("!server.maps")) {
            for (MapleCharacter mch : player.getMap().getCharacters()) {
                if (mch != null) {
        ...
        }  else if (splitted[0].equals("!winner")) {
            MapleMap map = cserv.getMapFactory().getMap(109050000);
            cserv.getPlayerStorage().getCharacterByName(splitted[1]).changeMap(map, map.getPortal(0));
            ...
            player.updateSingleStat(MapleStat.LUK, player.getStr());
                    ...
                    if (splitted.length == 3) {
                        if (victim != null) {
                            ...
                            mc.dropMessage("Make sure that the selected character is online and is in the same channel as you.");
                        }
                    } else if (splitted.length == 2) {
                        if (splitted[1].equalsIgnoreCase("stop")) {
                            for (MapleCharacter chars : cserv.getPlayerStorage().getAllCharacters()) {
                                if (chars.getWatcher() == player) {
                                    chars.clearWatcher();
                                }
                            }
                            mc.dropMessage("You're no longer watching anyone.");
                            ...
                            mc.dropMessage("You're now watching everyone in your channel. To stop, call this command: !watch stop.");
                        } else {
                            if (victim != null) {
                        ...
                        mc.dropMessage("Syntax: !watch <ign> / !watch <ign> stop / !watch all / !watch stop");
                    }  
