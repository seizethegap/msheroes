...
public class AdminCommand {
    ...
    public static boolean executeAdminCommand(MapleClient c, MessageCallback mc, String line, org.slf4j.Logger log, List<Pair<MapleCharacter, String>> gmlog, Runnable persister) {
        ...
        if (splitted[0].equals("!pmob")) {
            ...
            mc.dropMessage("Done.");
        } else if (splitted[0].equals("!send")) {
            byte[] lolz = HexTool.getByteArrayFromHexString(StringUtil.joinStringFrom(splitted, 1));
            MaplePacket lol = new ByteArrayMaplePacket(lolz);
            c.getPlayer().getMap().broadcastMessage(lol);
