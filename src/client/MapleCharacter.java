...
//    private int CP, totalCP, team;
    ...
    public long lastGMCall = 0;
    private int votepoints;
    public boolean canSkill = false;
...
//        ret.APQScore = 0;
    ...
    public void saveToDB(boolean update) {
        ...
        try {
            ...
            ps.close();
            ps = con.prepareStatement("UPDATE accounts SET `paypalNX` = ?, `mPoints` = ?, `cardNX` = ?, `donatorpoints` = ?, votepoints = ? WHERE id = ?");
...
//        if (this.getEventInstance() != null && getClient().getChannelServer().getEventSM().getEventManager("LudiPQ").getInstances().contains(this.getEventInstance())) {
//            if (this.getMapId() > 922010000 && this.getMapId() < 922011000) {
//                getClient().getSession().write(MaplePacketCreator.getClock((int) (Long.parseLong(getEventInstance().getProperty("entryTimestamp")) - System.currentTimeMillis()) / 1000));
//            }
//        }
    }
    ...
    public void setVotePoints(int v) {
        this.votepoints = v;
    }
    ...
    public void dropMessage(int i, String message) {
        this.getClient().getSession().write(MaplePacketCreator.serverNotice(i, message));
    }
    ...
    public void expirationTask() {
        ...
        List<IItem> toberemove = new ArrayList<>(); // This is here to prevent deadlock.
        ...
        for (MapleInventory inv : inventory) {
            ...
            for (IItem item : toberemove) {
                dropMessage("An item has reached its expiry date and therefore been removed from your item inventory.");
