...
public class MapleClient {
    ...
    public boolean changingMaps = false;
    ...
    public int login(String login, String pwd, boolean ipMacBanned) {
        ...
        Connection con = DatabaseConnection.getConnection();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT id,password,salt,tempban,banned,gm,macs,lastknownip,greason, pin FROM accounts WHERE name = ?");
            ...
            if (player.getMapId() > 2 && player.getLevel() > 1) {
                if (!player.isGM()) {
                    for (ChannelServer cs : ChannelServer.getAllInstances()) {
                        cs.broadcastPacket(MaplePacketCreator.serverNotice(6, player.getName() + " has left the game."));
                    }
                }
            }
            if (chr.hasFakeChar()) {
