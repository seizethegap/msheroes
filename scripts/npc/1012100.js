    ...
    if (mode == -1)
        ...
        if (status == 0) {
            if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER)) {
                ...
                if (cm.getLevel() >= 30 && cm.getJob().equals(Packages.client.MapleJob.BOWMAN)) {
                    if (cm.getQuestStatus(100000).getId() >= Packages.client.MapleQuestStatus.Status.STARTED.getId()) {
