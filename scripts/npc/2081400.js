...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.getJob().equals(Packages.client.MapleJob.CRUSADER)) {
                status = 126;
                cm.sendYesNo("Conratulations on reaching such a high level. Do you want to Job Advance now?");
            } else if (cm.getJob().equals(Packages.client.MapleJob.WHITEKNIGHT)) {
                status = 129;
                cm.sendYesNo("Conratulations on reaching such a high level. Do you want to Job Advance now?");
            } else if (cm.getJob().equals(Packages.client.MapleJob.DRAGONKNIGHT)) {
                status = 132;
                cm.sendYesNo("Conratulations on reaching such a high level. Do you want to Job Advance now?");
            }		
        } else if (status == 127) {
