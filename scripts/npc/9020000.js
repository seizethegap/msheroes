		...
		if (status == 0) {
			...
			if (!cm.isLeader()) { // not party leader
				...
				if (next) {
					...
					if (em == null) {
						cm.sendOk("This PQ is currently not available.");
					}
					else {
						// Begin the PQ.
					...
					cm.sendOk("Your party must be three to six members. Please make sure all your members are present and qualified to participate in this quest.  I see #b" + levelValid.toString() + " #kmembers are in the right level range, and #b" + inMap.toString() + "#k are in Kerning. If this seems wrong, #blog out and log back in,#k or reform the party.");
					cm.dispose();
				}
			}
		}
