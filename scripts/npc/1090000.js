    ...
    if (mode == -1)
        ...
        if (status == 0) {
            if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER)) {
                ...
                if (cm.getLevel() >= 30 && cm.getJob().equals(Packages.client.MapleJob.PIRATE)) {
                    status = 10;
                    cm.sendNext("The progress you have made is astonishing.");
                } else if (cm.getLevel() >= 70 && (cm.getJob().equals(Packages.client.MapleJob.BRAWLER) || cm.getJob().equals(Packages.client.MapleJob.GUNSLINGER)))
                    cm.sendOk("Please go visit #bArec#k. He resides in #bEl Nath#k.");
                else if (cm.getLevel() < 30 && cm.getJob().equals(Packages.client.MapleJob.PIRATE))
                    cm.sendOk("Please come back to see me once you have trained more.");
                else if (cm.getLevel() >= 120 && (cm.getJob().equals(Packages.client.MapleJob.GUNSLINGER) || cm.getJob().equals(Packages.client.MapleJob.OUTLAW)))
            ...
            if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER))
                cm.changeJob(Packages.client.MapleJob.PIRATE);
            ...
            if (selection == 0) {
                jobName = "Gunslinger";
                job = Packages.client.MapleJob.GUNSLINGER;
            } else {
                jobName = "Brawler";
                job = Packages.client.MapleJob.BRAWLER;
            }
