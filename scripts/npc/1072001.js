...
function action(mode, type, selection) {
    if (mode == -1)
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0 && cm.getQuestStatus(100007) == Packages.client.MapleQuestStatus.Status.STARTED)
            status = 3;
        if (status == 0) {
            if (cm.getQuestStatus(100006).getId() >= Packages.client.MapleQuestStatus.Status.STARTED.getId()) {
                cm.completeQuest(100006);
                if (cm.getQuestStatus(100006) == Packages.client.MapleQuestStatus.Status.COMPLETED)
		    cm.startQuest(100007);
		    cm.completeQuest(100007);
		    cm.startQuest(100008);
                    cm.sendOk("I'm glad you found your way here! Please get back to #bThe Old Grendel#k and you will be able to select which job you would like to advance to.");
		    cm.dispose();
            } else {
