...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0 && cm.getQuestStatus(100004) ==Packages.client.MapleQuestStatus.Status.STARTED) {
			status = 3;
		}
		if (status == 0) {
			if (cm.getQuestStatus(100004) == Packages.client.MapleQuestStatus.Status.COMPLETED) {
				...
				cm.dispose();
			} else if (cm.getQuestStatus(100003).getId() >=Packages.client.MapleQuestStatus.Status.STARTED.getId()) {
				cm.completeQuest(100003);
				if (cm.getQuestStatus(100003) ==Packages.client.MapleQuestStatus.Status.COMPLETED) {
