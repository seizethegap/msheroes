/*
	Dances with Balrog, Warrior Job Advancement
		...
		if (status == 0) {
			if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER)) {
				...
				if (cm.getLevel() >= 30 
					...
					if (cm.getQuestStatus(100003).getId() >= Packages.client.MapleQuestStatus.Status.STARTED.getId()) {
						cm.completeQuest(100005);
						if (cm.getQuestStatus(100005) == Packages.client.MapleQuestStatus.Status.COMPLETED) {
							status = 20;
