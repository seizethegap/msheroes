    ...
    if (mode == -1) 
        ...
        if (status == 0) {
            if (cm.getJob().equals(Packages.client.MapleJob.BEGINNER)) {
                ...
                if (cm.getLevel() >= 30 && cm.getJob().equals(Packages.client.MapleJob.THIEF)) {
                    if (cm.getQuestStatus(100009).getId() >= Packages.client.MapleQuestStatus.Status.STARTED.getId()) {
                        cm.completeQuest(100011);
                        if (cm.getQuestStatus(100011) == Packages.client.MapleQuestStatus.Status.COMPLETED) {
                            status = 20;
