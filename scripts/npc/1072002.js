...
function action(mode, type, selection) {
    if (mode == -1)
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0 && cm.getQuestStatus(100001) == Packages.client.MapleQuestStatus.Status.STARTED)
            status = 3;

        if (status == 0) {
            if (cm.getQuestStatus(100001) == Packages.client.MapleQuestStatus.Status.COMPLETED) {
                cm.sendOk("You're truly a hero!");
                cm.dispose();
            } else if (cm.getQuestStatus(100000).getId() >= Packages.client.MapleQuestStatus.Status.STARTED.getId()) {
                cm.completeQuest(100000);
                if (cm.getQuestStatus(100000) == Packages.client.MapleQuestStatus.Status.COMPLETED)
