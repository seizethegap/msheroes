...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.getJob().equals(Packages.client.MapleJob.FP_MAGE)) {
                status = 117;
                cm.sendYesNo("Conratulations on reaching such a high level. Do you want to Job Advance now?");
            } else if (cm.getJob().equals(Packages.client.MapleJob.IL_MAGE)) {
                status = 120;
                cm.sendYesNo("Conratulations on reaching such a high level. Do you want to Job Advance now?");
            } else if (cm.getJob().equals(Packages.client.MapleJob.PRIEST)) {
                status = 123;
                cm.sendYesNo("Conratulations on reaching such a high level. Do you want to Job Advance now?");
            }		
        } else if (status == 118) {
