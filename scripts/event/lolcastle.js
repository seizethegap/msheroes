...
function announce() {
	...
	em.getChannelServer().broadcastPacket( 
		tools.MaplePacketCreator.serverNotice(6, "[Event] Field of Judgement will open in " + i + " minutes"));
	i--;
}
...
function mesoDistribution() {
	...
	while (iter.hasNext()) {
		...
		if (eim.getPlayerCount() > 0) {
			...
			map.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(6, "[Event] " + winner.getName() + " wins " + meso + " meso"));
...
function startInstance(eim) {
	if (eim.getPlayerCount() > 0) {
		...
		if (eim.getName().equals("lolcastle1")) {
			for (var x = 0; x < 100; x++) {
				var mob = server.life.MapleLifeFactory.getMonster(9500169);
			...
			for (var x = 0; x < 10; x++) {
				var mob = server.life.MapleLifeFactory.getMonster(9300136);
				var overrideStats = new server.life.MapleMonsterStats();
			...
			for (var x = 0; x < 100; x++) {
				var mob = server.life.MapleLifeFactory.getMonster(5120000);
				var overrideStats = new server.life.MapleMonsterStats();
			...
			for (var x = 0; x < 3; x++) {
				var mob = server.life.MapleLifeFactory.getMonster(9300012);
				var overrideStats = new server.life.MapleMonsterStats();
			...
			for (var x = 0; x < 100; x++) {
				...
				var mob = server.life.MapleLifeFactory.getMonster(mobId);
				var overrideStats = new server.life.MapleMonsterStats();
			...
			for (var x = 0; x < 3; x++) {
				var mob = server.life.MapleLifeFactory.getMonster(9300105);
				var overrideStats = new server.life.MapleMonsterStats();
				overrideStats.setHp(mob.getHp());
				overrideStats.setExp(mob.getExp() / 4);
				overrideStats.setMp(mob.getMaxMp());
			...
			for (var x = 0; x < 3; x++) {
				var mob = server.life.MapleLifeFactory.getMonster(9300152);
				var overrideStats = new server.life.MapleMonsterStats();
...
function playerDead(eim, player) {
	...
	player.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(1, "You lost with " + eim.getKillCount(player) + " points."));
}
...
function allMonstersDead(eim) {
	...
	if (priceRand <= 35) {
		...
		*/
		...
		price = new Packages.client.Item(scrolls30[scrollRand], 0, 1);
		...
		price = new Packages.client.Item(powerUps[powerRand][0], 0, powerUps[powerRand][1]);
	} else if (priceRand > 70 && priceRand <= 75) {
		// throwing stars
		var starId = 2070003 + Math.floor(Math.random() * 14);
		price = new Packages.client.Item(starId, 0, ii.getSlotMax(starId));
		...
		price = new Packages.client.Item(scrolls60[scrollRand], 0, 1);
	} else {
		// 40% scroll
		...
		price = new Packages.client.Item(scrolls40[scrollRand], 0, 1);		
	}
	
	var iter = eim.getPlayers().iterator();
	while (iter.hasNext()) {
		var winner = iter.next();
		winner.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(1, "You win with " + eim.getKillCount(winner) + " points. You will be warped out in 2 minutes."));
		winner.getClient().getSession().write(Packages.tools.MaplePacketCreator.getClock(120));
